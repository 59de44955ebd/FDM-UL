...
//	void View_Language (LPCSTR pszLang);
	...
	void Webinterface_AutoStart (BOOL b);
	BOOL Webinterface_AutoStart();

	UINT Webinterface_Port();
	void Webinterface_Port(UINT n);

	void Webinterface_UsePwd (BOOL b);
	BOOL Webinterface_UsePwd();

	CString Webinterface_Login();
	void Webinterface_Login(LPCSTR psz);

	CString Webinterface_Password();
	void Webinterface_Password(LPCSTR psz);

	CString YtDlp_Path();
	void YtDlp_Path(LPCSTR psz);

public:
	BOOL m_bHasYtDlp;
	TCHAR m_szYtDlpPath[MAX_PATH] = "yt-dlp.exe";
